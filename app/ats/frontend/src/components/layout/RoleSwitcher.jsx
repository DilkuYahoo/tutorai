...
export default function RoleSwitcher() {
  ...
  // In API mode: show user name + logout, no role switching
  if (USE_API) {
    return (
      <div className="relative" ref={ref}>
        ...
        {open && (
          <div className="absolute right-0 mt-2 w-44 bg-slate-900 border border-slate-800 rounded-xl shadow-xl z-50 overflow-hidden animate-fade-in">
            <p className="px-3 py-2 text-xs text-slate-400 truncate">{currentUser.email}</p>
  ...
  // Mock mode: role switcher for local dev
