    ...
    <div className="max-w-xl mx-auto px-6 py-12 animate-fade-in">
      ...
      <div className="mb-8">
        <div className="flex justify-between mb-2">
          {STEPS.map((label, i) => (
            <span
              key={label}
              className={`text-xs font-medium ${i === step ? 'text-indigo-400' : i < step ? 'text-emerald-400' : 'text-slate-600'}`}
            >
              {label}
            </span>
          ))}
      ...
      {step === 2 && (
        <div className="space-y-4 animate-fade-in">
          ...
          />
          <p className="text-xs text-slate-500">This is optional. If you'd prefer, you can skip this step.</p>
        </div>
      )}
      ...
      <div className="flex justify-between mt-8">
        ...
        {step < STEPS.length - 1 ? (
          ...
          <BaseButton onClick={() => dispatch({ type: 'SUBMIT' })}>
            Submit Application
          </BaseButton>
        )}
      </div>
