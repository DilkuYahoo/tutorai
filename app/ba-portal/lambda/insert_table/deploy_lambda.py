#!/usr/bin/env python3
"""
Simplified AWS Lambda Deployment Script for DynamoDB Insert Function
"""
...
class LambdaDeployer:
    ...
    def get_s3_bucket(self):
        ...
        if s3_bucket:
            self.logger.info(f"Using S3 bucket: {s3_bucket}")
        else:
            self.logger.info("No S3 bucket configured")
        return s3_bucket
        ...
        try:
            with zipfile.ZipFile(package_name, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                if os.path.exists(libs_dir) and os.path.isdir(libs_dir):
                    self.logger.info(f"Found libs directory: {libs_dir}")
                    ...
                    self.logger.info(f"No libs directory found at: {libs_dir}")
    ...
    def attach_dynamodb_policy(self, role_name):
        ...
        # Custom policy for DynamoDB insert access
        dynamodb_policy = {
            "Version": "2012-10-17",
            "Statement": [
                {
            ...
            try:
                ...
                if e.response['Error']['Code'] == 'NoSuchEntity':
                    ...
                    policy_response = self.iam_client.create_policy(
                        ...
                        PolicyDocument=json.dumps(dynamodb_policy),
                        Description=f'DynamoDB insert access policy for {role_name}'
                    )
