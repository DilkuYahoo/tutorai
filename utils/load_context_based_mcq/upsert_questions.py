...
class TextAssessmentGenerator:
    ...
    def get_text_input(self) -> Optional[Tuple[str, str, str, str]]:
        ...
        if choice == "1":
            ...
            text = self.get_text_direct_input()

        if not text:
            return None

        # Prompt for year
        year = input("Enter the year (e.g., 12 for HSC Year 12): ").strip()
        if not year:
        ...
        if not area:
            area = "unseen_text"  # Default area
            logger.warning("No area provided, using default: unseen_text")
...
def setup_user_input(generator: TextAssessmentGenerator) -> Optional[Tuple[str, str, str, str, str]]:
    """Handle user input collection with proper error handling"""
    try:
        print("🎓 HSC English Text Assessment Generator")
        print("=" * 50)

        input_result = generator.get_text_input()
        if not input_result:
