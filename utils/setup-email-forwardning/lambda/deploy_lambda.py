...
class LambdaDeployer:
    def __init__(self, config_file='deploy.config'):
        ...
        self.region = self.config.get('region', 'ap-southeast-2')
        self.cleanup_files = []
    ...
    def load_config(self, config_file):
        ...
        defaults = {
            'function_name': 'email-forwarder-lambda-function',
            'region': 'us-east-1',
            'runtime': 'python3.13',
            'handler': 'email-forwarder.lambda_handler',
            'memory': '256',
            'timeout': '30',
            'role_name': 'email-forwarder-lambda-function-role',
            'script_path': 'email-forwarder.py',
            'requirements_file': 'requirements.txt',
            'layers': '',
            'layer_arns': '',
            's3_bucket': 'cognifylabs-ai-emails',
            'env_vars': ''
        }
    ...
    def attach_s3_policy(self, role_name):
        """Attach S3 access policy to the IAM role for email storage"""
        ...
        s3_bucket = self.config.get('s3_bucket', 'cognifylabs-email-bucket')

        # Custom policy for S3 access (email storage and retrieval)
        s3_policy = {
            "Version": "2012-10-17",
            "Statement": [
                {
    ...
    def attach_ses_policy(self, role_name):
        """Attach SES access policy to the IAM role for email sending"""
        if not self.get_client('iam'):
            return False

        account_id = self.get_account_id()
        if not account_id:
            print("❌ Could not get AWS account ID for SES policy")
            return False
        ...
        policy_name = f"{role_name}-ses-policy"

        try:
            # Check if policy already exists
            try:
                existing_policy = self.iam_client.get_policy(PolicyArn=f"arn:aws:iam::{account_id}:policy/{policy_name}")
                print(f"SES policy {policy_name} already exists")
                ...
                if existing_document != ses_policy:
                    print(f"Policy {policy_name} has outdated permissions, updating...")
                    # Detach old policy
                    try:
                        self.iam_client.detach_role_policy(
                        ...
                        print(f"Detached old SES policy from role: {role_name}")
                    except ClientError as e:
                        if e.response['Error']['Code'] != 'NoSuchEntity':
                            print(f"Warning: Could not detach old policy: {e}")
                    ...
                    print(f"Updated SES policy: {policy_name}")
                else:
                    print(f"Policy {policy_name} is up to date")
                ...
                if e.response['Error']['Code'] == 'NoSuchEntity':
                    # Create the policy
                    print(f"Creating SES policy: {policy_name}")
                    policy_response = self.iam_client.create_policy(
                        PolicyName=policy_name,
                        PolicyDocument=json.dumps(ses_policy),
                        Description=f'SES access policy for {role_name}'
                    )
                    print(f"✅ Created SES policy: {policy_response['Policy']['Arn']}")
                else:
                    print(f"❌ Error checking SES policy: {e}")
                    return False

            # Attach the policy to the role
            print(f"Attaching SES policy to role: {role_name}")
            try:
                self.iam_client.attach_role_policy(
                    RoleName=role_name,
                    PolicyArn=f"arn:aws:iam::{account_id}:policy/{policy_name}"
                )
                print(f"✅ Attached SES policy to role: {role_name}")
                return True
            except ClientError as e:
                if e.response['Error']['Code'] == 'PolicyAlreadyAttached':
                    print(f"SES policy already attached to role: {role_name}")
                    return True
                else:
                    print(f"❌ Error attaching SES policy: {e}")
                    return False
    ...
    def create_iam_role(self):
        ...
        try:
            ...
            if not self.attach_ses_policy(role_name):
                print(f"⚠️  Warning: Failed to attach SES policy to role {role_name}")
    ...
    def deploy_lambda(self, destroy_existing=False):
        ...
        try:
            ...
            if env_vars:
                ...
                if env_dict:
                    function_config['Environment'] = {'Variables': env_dict}
                    print(f"Setting {len(env_dict)} environment variables")
                    ...
                    if env_vars:
                        ...
                        for env_pair in env_vars.split(','):
                            if '=' in env_pair:
                                ...
                                env_dict[key.strip()] = value.strip()
                        if env_dict:
                            update_config['Environment'] = {'Variables': env_dict}
